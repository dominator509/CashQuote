...
The subsequent pre-ship audit findings AUD-001 through AUD-003 and AUD-005 through AUD-010 were addressed in the repository. AUD-004 is closed for repository CI evidence on source-code hardening SHA `8b47539630f61c4eecb5b0f015c781dc42833a2e`: GitHub Actions run 33470503447 passed the Docker image build and boot/readiness check, concurrency smoke, production smoke, dependency audit, the full Jest gate, and the four-test E2E gate. The source updates bind production pilot credentials to configured emails, redact session headers from request logs, bound readiness-probe traffic, remove client addresses from mock-mail logs, and require direct TLS for production SMTP while rejecting unsafe transport flags. Local SMTP-capture and PostgreSQL concurrency checks pass, including the complete reminder path from client lookup through sent-state persistence. Real provider SMTP acceptance, deployment-topology validation, branch protection, and Git-history secret-scan evidence remain deployment or repository-administration gates before public release.
